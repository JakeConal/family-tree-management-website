      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[1440px]">
        ...
        <main className="box-border w-[1232px] max-w-[1232px] flex-1 bg-white px-10 pb-16 pt-10" style={{ minHeight: "1024px" }}>
            ...
            <div className="mt-6 flex justify-center">
              <div className="flex w-full max-w-[1032px] overflow-hidden rounded-full border border-[#B9BEC7] bg-[#E7E8EB] shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]">
                {achievementTabs.map((tab, index) => (
                    ...
                    className={`flex flex-1 items-center justify-center py-3 text-base font-semibold transition ${
                      activeTab === tab.id
          ...
          <section className="mt-10 space-y-12">
            {isPassingView
              ? passingSections.map(({ year, entries }) => (
                  <div key={year}>
                    ...
                    <div className="mx-auto flex max-w-[1032px] flex-col gap-[30px]">
                      {entries.map((entry) => {
                        ...
                        return (
                          <article
                            key={entry.id}
                            className="flex h-[152px] w-full flex-col justify-between rounded-[24px] border border-[#B4B4B4] bg-[#D9D9D9] px-6 py-5 text-[#1A1A1A] shadow-[0_6px_12px_rgba(0,0,0,0.15)]"
                          >
              ...
              : isLifeEventView
                ? lifeEventSections.map(({ year, entries }) => (
                    <div key={year}>
                      <div className="mb-6 flex items-center gap-4">
                      ...
                      <div className="mx-auto flex flex-col gap-[30px] lg:grid lg:grid-cols-[repeat(2,554px)] lg:justify-between lg:gap-x-[44px] lg:gap-y-[30px]">
                        {entries.map((entry) => (
                          <article
                            key={entry.id}
                            className="flex h-[152px] w-[554px] flex-col justify-between rounded-[30px] border border-black/25 bg-white px-6 py-5 text-[#231F20] shadow-[0_6px_12px_rgba(0,0,0,0.15)]"
