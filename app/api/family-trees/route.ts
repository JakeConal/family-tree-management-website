...
export async function POST(request: NextRequest) {
	try {
		...
		if (!rootPerson?.fullName || typeof rootPerson.fullName !== 'string' || rootPerson.fullName.trim().length === 0) {
			return NextResponse.json({ error: 'Root person full name is required' }, { status: 400 });
		}
		...
		await logChange('FamilyTree', familyTree.id, 'CREATE', familyTree.id, session.user.id, null, {
		...
		if (rootPerson.profilePicture && typeof rootPerson.profilePicture === 'string') {
			// Remove data URL prefix if present (e.g., "data:image/jpeg;base64,")
			const base64Data = rootPerson.profilePicture.replace(/^data:image\/[a-z]+;base64,/, '');
		...
		await logChange('FamilyMember', rootMember.id, 'CREATE', familyTree.id, session.user.id, null, {
