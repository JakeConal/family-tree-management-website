...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		// Fetch all passing records for this family tree
		...
		return NextResponse.json(passingRecords);
	} catch (error) {
	...
	try {
		...
		for (const place of burialPlaces) {
			if (!place.location || !place.startDate) {
				return NextResponse.json({ error: 'All burial places must have location and start date' }, { status: 400 });
			}
		}
		...
		await logChange('PassingRecord', passingRecord.id, 'CREATE', familyTreeId, session.user.id, null, {
